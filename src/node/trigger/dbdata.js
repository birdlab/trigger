...
function getFullTracksFromQery(qq, callback, dt) {
	db.connection.query(qq, function (error, result, fields) {
		if (!error) {
			...
			for (var t in result) {
				...
				tracks.push({
					artist: result[t].artist.replace('&amp;', '&'),
					title: result[t].title.replace('&amp;', '&'),
					info: result[t].info,
					playdate: result[t].playdate,
					path: result[t].path,
					positive: [],
					negative: [],
					tags: [],
					time: result[t].time,
					id: result[t].id,
					channel: result[t].channel,
					submiter: result[t].submiter,
					name: result[t].name,
					gold: result[t].gold > 0,
					rating: 0
				});
				ids += result[t].id + ',';
			}
...
exports.getTracksByShift = function (data, callback) {
	...
	if (!data.top) {
		...
		date = 'date'
		var order = ' ORDER BY tracks.rating DESC, tracks.realrating DESC, tracks.' + date + '  DESC ';
		if (data.votes) {
			order = ' ORDER BY tracks.realrating DESC, tracks.rating DESC, tracks.' + date + ' DESC ';
		}
...
exports.getTrackByPath = function (path, callback) {
	var hour12limit='and tracks.date BETWEEN NOW() - INTERVAL 12 HOUR AND NOW()'
	//var hour12limit='';
	var q = 'SELECT tracks.*, users.name FROM tracks LEFT JOIN users ON tracks.submiter=users.id WHERE tracks.path LIKE "%' + path + '%" ' + hour12limit+ ' order by tracks.date desc LIMIT 1';
	getFullTracksFromQery(q, callback);
};
...
exports.setOnDisk = function (id) {
	var q = "UPDATE tracks SET ondisk = " + db.connection.escape(1) + " WHERE id = " + db.connection.escape(id);
	db.connection.query(q, function (error, result, fields) {
		if (error) {
			console.log(error);
		}
...
function getRotationTracks(chid, tids, callback, searchCaseState, goldhours) {
	if (goldhours == 0) {
		goldhours = 1;
	}
	if (goldhours && goldhours > 24) {
		goldhours = 24;
	}
	var ps = '"' + tids.join('","') + '"';

	var excludeTime = ' and tracks.playdate not BETWEEN NOW() - INTERVAL ' + goldhours + ' HOUR AND NOW()';
	var order=' order by playdate ASC, rating DESC limit 1';
	var searchCase = [];
	...
	searchCase.push(' ');
	var q = 'SELECT tracks.*, users.name FROM tracks LEFT JOIN users ON tracks.submiter=users.id WHERE tracks.channel= ' + chid + ' AND tracks.ondisk=1 AND tracks.id not in (' + ps + ')' + searchCase[searchCaseState] + excludeTime+ order;
	db.connection.query(q, function (error, result, fields) {
		if (!error) {
			if (result.length > 0) {
				var ids = '';
				...
				//search more abstract query if no tracks found
				if (searchCaseState < searchCase.length - 1) {
					searchCaseState++;
					getRotationTracks(chid, tids, callback, searchCaseState, goldhours);
				} else {
					callback([]);
				}
			}
...
exports.getRotation = function (chid, tids, goldhours, callback) {
	getRotationTracks(chid, tids, callback, 0, goldhours);
};
...
exports.getChannelGoldSize = function (chid, callback) {
	channelGoldSize(chid, callback);
}

function channelGoldSize(chid, callback) {
...
function deleteGold(track, callback) {
	...
	fs.unlink('/home/trigger/upload' + track.path, function (err) {
		...
		db.connection.query('UPDATE tracks SET gold=1, ondisk=0 WHERE  id = "' + track.id + '" LIMIT 1', function (error, result, fields) {
			callback(track);
		});
	});
}
...
exports.getGoldTracks = function (paths, callback) {
	var ps = '"' + paths.join('","') + '"';
	var q = 'SELECT tracks.*, users.name FROM tracks LEFT JOIN users ON tracks.submiter=users.id WHERE path in (' + ps + ') and tracks.gold=1 order by tracks.date desc';
	db.connection.query(q, function (error, result, fields) {
		if (!error) {
			var ids = '';
			for (var t in result) {
...
exports.sendinvite = function (code, mail, socket) {
	var banlist = ['mailinator.com', 'trbvm.com', 'mail.org', 'dropmail.me', 'yomail.info', '10mail.org'];
	var fake = false;
	for (var i in banlist) {
		if (mail.search(banlist[i]) != -1) {
			fake = true;
			break;
		}
	}
				...
				if (result.length) {
					...
					db.connection.query('insert into invites (parentid, email, code, give_time,send_time, userid) VALUES ( 0,\'' + data.mail + '\', "' + code + '", NOW(), NOW(), 0)', function (er, result, fields) {
						if (!er) {
							...
							var fake = false;
							for (var i in banlist) {
								if (data.mail.search(banlist[i]) != -1) {
									fake = true;
									break;
								}
							}
