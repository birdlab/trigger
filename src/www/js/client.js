...
Client.prototype.init = function(host) {
	...
	socket.on('trackstatus', function(data){
		cl.callbacks.trackstatus(data);
	});
...
Client.prototype.tracksubmit = function(data, callback) {
	this.callbacks.trackstatus=callback;
	this.socket.emit('tracksubmit',{'chid':this.channel.chid, 'track':data.track});
}
...
Client.prototype.getUser = function(data, callback) {
	cl=this;
	cl.callbacks.userdata=callback;
	this.socket.emit('getuser',data);
}
Client.prototype.getHistory = function(shift, callback) {
	cl=this;
	cl.callbacks.history=callback;
	this.socket.emit('gethistory',{chid:cl.channel.chid, s:shift});
}
